...
namespace Proto
{
    static class Program
    {
        ...
        private const string NAME_SPACE = "Proto.ProtoInterface";
        private const string IGNORE_TYPE_END = "Base";
        private static readonly string NameSpaceLine =
            $"option csharp_namespace = '{NAME_SPACE}';";
        private static string _protoDir;
        private static string _csDir;
        private static string _csBaseDir;
        ...
        static void Main()
        {
            ...
            MoveProtoToProtoBase();

            Console.ReadLine();
        }

        private static void MoveProtoToProtoBase()
        {
            RewriteIfExist(_csDir, _csBaseDir);
        }

        private static void RewriteIfExist(string src, string target)
        {
            var fileSrc = new FileInfo(src);
            var fileTarget = new FileInfo(target);
            if (fileSrc.Exists && fileTarget.Exists)
            {
                fileSrc.CopyTo(target, true);
            }
            var dirSrc = new DirectoryInfo(src);
            var dirTarget = new DirectoryInfo(target);
            if (dirSrc.Exists && dirTarget.Exists)
            {
                foreach (var subFile in dirSrc.GetFiles())
                {
                    RewriteIfExist(
                        Path.Combine(src, subFile.Name), 
                        Path.Combine(target, subFile.Name));
                }
                foreach (var subDir in dirSrc.GetDirectories())
                {
                    RewriteIfExist(
                        Path.Combine(src, subDir.Name),
                        Path.Combine(target, subDir.Name));
                }
            }
        }
        ...
        private static void GeneMsgId()
        {
            ...
            {
                if (t.Name.EndsWith(IGNORE_TYPE_END))
                {
                    continue;
                }
                ...
                {
                    MsgIds.Instance.TcpReq[tcpReqIndex++] = $"{NAME_SPACE}.{GetFileName(t)}";
                    continue;
                }
                if (t.IsSubclassOf(typeof(TcpResBase)))
                {
                    MsgIds.Instance.TcpRes[tcpResIndex--] = $"{NAME_SPACE}.{GetFileName(t)}";
                    continue;
                }
                if (t.IsSubclassOf(typeof(HttpReqBase)))
                {
                    MsgIds.Instance.HttpReq[httpReqIndex++] = $"{NAME_SPACE}.{GetFileName(t)}";
                    continue;
                }
                if (t.IsSubclassOf(typeof(HttpResBase)))
                {
                    MsgIds.Instance.HttpRes[httpResIndex--] = $"{NAME_SPACE}.{GetFileName(t)}";
                    continue;
                }
            }
        ...
        private static void GeneProtos()
        {
            ...
            foreach (var type in types)
            {
                ...
                if (type.Name.EndsWith(IGNORE_TYPE_END))
                {
                    continue;
                }
                if (type.IsEnum)
